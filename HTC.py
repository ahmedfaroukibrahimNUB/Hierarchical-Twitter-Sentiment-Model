...
################################
df= pd.read_csv("C:\\Users\\Ahmed\\PycharmProjects\\SA\\final_train_data.csv")
df=df.drop_duplicates('text3')
df.info()
###############################################
# The output is details of final train data.csv
###############################################
df['neg'] = np.nan
df['neu'] = np.nan
df['pos'] = np.nan
...
plt.show()
########################################
# The output is image of wordcloud
########################################
################################################
...
df['clusters'] = clusters
#df.to_csv('clusters.csv', index=False)   # to determine distribution of  each tweets in clusters from 1 to 6
##############################################
...
dff.to_csv('Polarity_Score.csv', index=False)
df.to_csv('Sentiment_Polarity&Clusters.csv', index=False)
